...
async def role_choose(message: types.Message, state: FSMContext):
    if message.text.lower() not in presence:
        ...
        return
    # elif message.text.lower() == presence[1]:
    #     await message.answer(f"Ждем тебя до 18 января. После посещения подвтерди кнопкой ниже")
    #     await FaneronUsers.waiting_for_presence_accept.set()
    #     return
...
async def city_choose(message: types.Message, state: FSMContext):
    ...
    for c in city:
        keyboard.add(c)
    await FaneronUsers.next()
    tmp_state = await state.get_data()
    print(tmp_state)
...
async def get_user_data(message: types.Message, state: FSMContext):
    if message.text.lower() not in city:
        await message.answer(f"Выбери из предложенных вариантов городов")
        return
...
def register_faneron_users_handler(dp: Dispatcher, admin_id: int):
    ...
    dp.register_message_handler(users_statistic_start, commands='check_in', state='*')
    dp.register_message_handler(role_choose, state=FaneronUsers.waiting_for_presence_accept)
